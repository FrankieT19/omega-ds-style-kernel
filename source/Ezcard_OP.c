...
void IWRAM_CODE Check_FW_update(u16 Current_FW_ver,u16 Built_in_ver)
{
    ...
    sprintf(msg,"DS Style expects firmware V%02d.",Built_in_ver);
    DrawHZText12(msg,0,2,offset_Y+2*line_x, 0x7FFF,1);
    sprintf(msg,"Please update with the official");
    DrawHZText12(msg,0,2,offset_Y+4*line_x, 0x7FFF,1);
    sprintf(msg,"EZ-Flash Omega kernel first.");
    DrawHZText12(msg,0,2,offset_Y+5*line_x, 0x7FFF,1);
    sprintf(msg,"Press (B) to continue.");
    DrawHZText12(msg,0,2,offset_Y+7*line_x, 0x7FFF,1);
